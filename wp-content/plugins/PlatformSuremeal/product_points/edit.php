...
$id = (int)($_GET['id']);
$myrows = $wpdb->get_row("SELECT * FROM product_points WHERE id=" . $id);
...
$sql = "SELECT * FROM product_points WHERE id = $id";
$product_points = $wpdb->get_results($sql);
// var_dump($product_points);
// echo $product_points[0]->id_value;

$sqlProduct = "SELECT * FROM wp_posts WHERE post_type = 'product' AND post_status = 'publish'";
$products = $wpdb->get_results($sqlProduct);
                    ...
                    <div class="postbox">
                        ...
                        <div class="inside">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="nameProduct" name="nameProduct" value="<?= $myrows->name ?>" placeholder="Enter name">
                            </div>
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select w-100" id="productType" name="type" aria-label="Default select example">
                                        ...
                                        <option value="product" <?php if ($product_points[0]->type == 0) :?>  selected<?php endif;?>>Product</option>
                                        <option value="voucher" <?php if ($product_points[0]->type == 1) :?>  selected<?php endif;?>>Voucher</option>
                                        <option value="gift" <?php if ($product_points[0]->type == 2) :?>  selected<?php endif;?>>Gift</option>
                                ...
                                <div class="col-md-4 ">
                                    ...
                                    <select class="form-select w-100" name="productID" id="productID" aria-label="Default select example">
                                        <?php if ($product_points[0]->type == 0): ?>
                                            <?php foreach($products as $product): ?>
                                                <option value="<?= $product->ID ?>" <?= ($product_points[0]->id_value == $product->ID) ? 'selected' : ''; ?>><?= $product->post_title ?></option>
                                            <?php endforeach; ?>
                                        <?php elseif ($product_points[0]->type == 1): ?>
                                            <?php foreach($vouchers as $voucher): ?>
                                                <option value="<?= $voucher->id ?>" <?= ($product_points[0]->id_value == $voucher->id) ? 'selected' : ''; ?>><?= $voucher->voucher_name ?></option>
                                            <?php endforeach; ?>
                                        <?php elseif ($product_points[0]->type == 2): ?>
                                            <?php foreach($gifts as $gift): ?>
                                                <option value="<?= $gift->ID ?>" <?= ($product_points[0]->id_value == $gift->ID) ? 'selected' : ''; ?>><?= $gift->post_title ?></option>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <option >-----------------</option>
                                            <option selected value="<?= $myrows->id_value ?>">Product</option>
                                        <?php endif ?>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select w-100" name="status" id="status" aria-label="Default select example">
                                        ...
                                        <option value="1" <?php if ($myrows->status == 1) :?> selected<?php endif;?>>Active</option>
                                        <option value="0" <?php if ($myrows->status == 0) :?> selected<?php endif;?>>Not activated</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label for="note" class="form-label">Exchange points</label>
                                <input type="number" class="form-control" id="exchange_points" name="exchange_points" value="<?= $myrows->point ?>" placeholder="Enter points">
                            </div>
                            <div class="mb-3">
                                <label for="note" class="form-label">Note</label>
                                <textarea class="form-control" id="note" name="note" rows="3" placeholder="Enter note"><?= $myrows->note ?></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="inputGroupFile01">Upload image</label>
                                <input type="file" name="image" accept="image/png, image/jpeg" value="<?= $myrows->image ?>" class="form-control" id="inputGroupFile01">
                            </div>
                            <div class="mb-3">
                                <label for="purchases">Purchases</label>
                                <input type="number" class="form-control" id="purchases" name="purchases" value="<?= $myrows->purchases ?>" placeholder="Enter purchases">
                            </div>
                            <div class="row mb-3">
                                ...
                                <div class="mb-3">
                                    <input type="text" disabled class="form-control" value="<?= date('m/d/Y', $myrows->created_at) ?> - <?= date('m/d/Y', $myrows->expiration_date) ?>" placeholder="" id="dates" />
                                </div>
                                <input type="hidden" value="<?= date('m/d/Y', $myrows->created_at) ?>" id="created_at" name="created_at" />
                                <input type="hidden" value="<?= date('m/d/Y', $myrows->expiration_date) ?>" name="expiration_date" id="expiration_date"  />
                            </div>
                        </div>
    ...
    jQuery(document).ready(function() {
        jQuery('#productType').on('change', function () {
            ...
            if (selectedType === 'product') {
                ...
                <?php foreach($products as $product): ?>
                    jQuery('#productID').append('<option value="<?= $product->ID ?>" <?php if ($product_points[0]->id_value == $product->ID) :?>  selected<?php endif;?>><?= $product->post_title ?></option>');
                <?php endforeach; ?>
            } else if (selectedType === 'voucher') {
                jQuery('#productID').append('<option selected>Select voucher</option>');
                <?php foreach($vouchers as $voucher): ?>
                    jQuery('#productID').append('<option value="<?= $voucher->id ?>" <?php if ($product_points[0]->id_value == $voucher->id) :?>  selected<?php endif;?>><?= $voucher->voucher_name ?></option>');
                <?php endforeach; ?>
        ...
        jQuery('#publish').on('click', function () {
            ...
            formData.append('action', 'edit_points');
