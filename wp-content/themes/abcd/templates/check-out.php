...
<script defer>
    ...
    function getShippingFee(weight, shippingData) {
        console.log(shippingData);
        if (weight <= parseFloat(shippingData[0].weight_range)) {
            // Nếu khối lượng nhỏ hơn hoặc bằng khoảng đầu tiên
            return parseFloat(shippingData[0].shipping_fee);
        }

        for (let i = 0; i < shippingData.length - 1; i++) {
            const currentRange = parseFloat(shippingData[i].weight_range);
            const nextRange = parseFloat(shippingData[i + 1].weight_range);

            if (weight > currentRange && weight <= nextRange) {
                // Nếu khối lượng nằm trong khoảng
                return parseFloat(shippingData[i + 1].shipping_fee);
            }
        }

        // Nếu khối lượng lớn hơn khoảng cuối cùng
    ...
    function calculateShipping(selectedCountry, selectedState = '') {
        ...
        if (countryData) {
            ...
            if (selectedCountry === 'United States') {
                ...
                const shipping_weightConty = countryData.shipping_weight;
