...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $emailExistsInAccount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM wp_account_dealers WHERE email = %s", $email));

    // Kiểm tra email đã tồn tại trong wp_pending_dealers
    ...
    if (!empty($email)) {
        if ($emailExistsInAccount > 0) {
            ...
            if ($pendingUser->days_diff >= 1) {
                ...
                $wpdb->delete('wp_pending_dealers', array('email' => $email));
    ...
    if (empty($errors)) {
        ...
        // Thêm dữ liệu vào bảng wp_pending_dealers
        $table = 'wp_pending_dealers';
        $data = array(
            'first_name' => $firstName,
            'last_name' => $lastName,
            'email' => $email,
        ...
        <div class="mt-10 xl:mt-12 w-full mx-auto rounded-2xl border border-solid border-neutral-200 bg-white">
            <div class="p-8 flex flex-col gap-8">
                <div class="flex gap-6 items-center">
                    <a href="<?= home_url() ?>/sign-in-dealer" class="sign-tab">Login</a>
                ...
                <div class="flex flex-col items-center gap-6">
                    <form id="signUpForm" class="w-full" method="post">
                        <div class="flex flex-wrap gap-4">
                            ...
                            <button id="submit-sign-up" type="submit" class="mx-auto min-w-[162px] button bg-primary text-body-md-semibold text-white"><?php pll_e('Sign up') ?></button>
                                ...
                                <div class="relative w-full max-w-3xl max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 p-10">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <a href="<?= home_url() ?>" class="text-[#FF0000] border-[#FF0000] rounded-lg w-full sm:w-auto px-28 py-3 text-center no-underline" style="background: #FFF; font-weight: 600; border: 1px solid;">Sign up now</a>
                                            <a href="<?= home_url() ?>" class="text-white border-none rounded-lg w-full sm:w-auto px-28 py-3 text-center no-underline" style="background: #ED1B24; font-weight: 600;">Maybe later</a>
                                        </div>
                                    </div>
                                </div>
                            </div> -->
                    ...
                    <p class="text-body-md-regular text-gray-9"><?php pll_e('Already have an account?') ?> <a href="<?= home_url() ?>/sign-in-dealer"
