...
$user = $wpdb->get_row($wpdb->prepare(
    "SELECT last_resend_date FROM wp_pending_dealers WHERE email = %s",
    $email
));
$remaining_seconds = 0;
if ($user) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pending_user = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM wp_pending_dealers 
        WHERE email = %s 
        AND otp = %s 
    ...
    if ($pending_user) {
        ...
        $wpdb->insert('wp_account_dealers', array(
        ...
        wp_send_json_success(array('message' => 'You have successfully registered as a dealer. Your registration request is pending approval by SureMeal.'));
    } else {
...
var initialTimeLeft = <?php echo $remaining_seconds; ?>;
    ...
    <div class="relative min-h-screen overflow-hidden bg-gray-50 py-16 sm:py-12 reset-pass text-center px-4">
        <div class="mx-auto items-center mb-16">
            <a href="<?= home_url() ?>"><img src="<?= $url ?>/assets/dealer/img/logo.png" class="w-64" alt="" /></a>
        </div>
        <div class="mx-auto relative p-8 rounded-2xl bg-white w-500-full overflow-hidden border-cm">
            ...
            <form class="" action="#" method="">
                ...
                <div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative w-full max-w-3xl max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 p-10">
                            ...
                            <p class="text-center text-base mb-8 signup-color-1 font-normal">You have successfully registered as a dealer. Your registration request is pending approval by SureMeal. Please check your email for the approval result and to log in to the website.</p>
                ...
                <div id="modal-overlay" class="bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-40 hidden"></div>
            </form>
        </div>
    </div>
    <script>
