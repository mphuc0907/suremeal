...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profile'])) {
    ...
    if ($password_change_attempted) {
        ...
        if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            // Nếu không có mật khẩu (null) hoặc provider khác rỗng
            if ($password_is_null || $provider != '') {
                // Validate mật khẩu mới khi tạo mới
            ...
            // Nếu đang thay đổi mật khẩu cho tài khoản có sẵn mật khẩu
            else {
                ...
                <div class="w-full lg:max-w-[322px] flex flex-col gap-5">
                    ...
                    <div class="dashboard-container">
                        <a href="<?= home_url() ?>/person-info" class="dashboard-item active">
                            ...
                            <p class="text-body-md-medium">Personal information</p>
                        </a>
                        <a href="<?= home_url() ?>/order-info" class="dashboard-item">
                            ...
                            <p class="text-body-md-medium">Order infomation</p>
                        </a>
                        <form class="dashboard-item" id="logoutForm" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST">
                            ...
                            <button type="submit" class="text-body-md-medium logout">Log Out</button>
                        </form>
                    </div>
                </div>
                <div class="flex-1">
                    <div class="rounded-xl bg-white pb-10 flex flex-col gap-12">
                        <div class="px-8 py-4 border-b border-solid border-neutral-200">
                            <h2 class="text-body-xl-regular text-gray-8">Personal information</h2>
                        </div>
                        ...
                        <div class="info-wrap flex items-center justify-center">
                            ...
                            <form method="POST" enctype="multipart/form-data" class="form-edit">
                                ...
                                <div
                                    class="edit-info hidden w-full max-w-[455px] px-4 lg:px-0 flex flex-col items-center justify-center gap-8">
                                    ...
                                    <div class="w-full flex flex-col gap-5">
                                        ...
                                        <label class="email-edit input-label">
                                            <p class="input-title text-neutral-500">Email</p>
                                        ...
                                        <div class="address-edit w-full flex flex-col gap-3">
                                            <div id="address-container" class="w-full flex flex-col gap-3">
                                                <label class="address-edit input-label">
                                                    <p class="input-title text-neutral-500">Address</p>
                                        ...
                                        // $show_password_section = ($provider != '' && $password_is_null) || $provider == '';
                                        ?>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        updateButton.addEventListener('click', function(event) {
            ...
            // Kiểm tra nếu có nhập một trong các trường mật khẩu
            const passwordInputs = [currentPasswordInput, newPasswordInput, confirmPasswordInput];
            const passwordFieldsFilled = passwordInputs.some(input => input && input.value.trim() !== '');
            
            if (passwordFieldsFilled) {
                // Kiểm tra xem tất cả các trường có được điền đầy đủ không
                const allPasswordFieldsFilled = passwordInputs.every(input => 
                    input && input.value.trim() !== ''
                );

                if (!allPasswordFieldsFilled) {
                    // Tạo thông báo lỗi cho các trường mật khẩu
                    const passwordErrorMessage = 'Please fill in all password fields';
                    const errorElement = document.createElement('p');
                    errorElement.classList.add('validation-error', 'text-[#FF0000]', 'text-sm', 'mt-1');
                    errorElement.textContent = passwordErrorMessage;

                    // Chèn thông báo lỗi sau input mật khẩu cuối cùng
                ...
                // Kiểm tra mật khẩu hiện tại
                if (!hasError) {
                    // Sử dụng AJAX để kiểm tra mật khẩu hiện tại
                ...
                // Nếu không có lỗi và không thay đổi mật khẩu, submit form ngay
                form_edit.submit();
            }
        });
        ...
        function createErrorMessage(inputElement, message) {
            ...
            // Chèn thông báo lỗi ngay sau input
